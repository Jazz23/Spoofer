...
namespace Spoofer
{
    public class Spoofer : IPlugin
    {
        ...
        public void Initialize(Proxy proxy)
        {
            ...
            proxy.HookPacket<NewTickPacket>(OnNT);
        }
        ...
        {
            //if (Name != null && client.PlayerData.Name == Name) return;
                ...
                foreach (StatData statt in ent.Status.Data)
                {
                    if (statt.Id == StatsType.Name && statt.StringValue == Name)
                    {
                        ...
                        foreach (StatData stat in ent.Status.Data)
                        {
                            ...
                            else if (stat.Id == StatsType.MaximumHP && MaximumHP != 0) { PrevHealth = stat.IntValue; stat.IntValue = MaximumHP; }
                            else if (stat.Id == StatsType.MaximumMP && MaximumMP != 0) { PrevMana = stat.IntValue; stat.IntValue = MaximumMP; }
